...
For the $6$ and $12$ particle quantum dots the evaluation of the first derivatives single particle orbitals is the most time consuming part of the code.
...
%The problem is that half of these derivatives are calculated each time one particle is moved. It does not pay off storing them since the number of derivatives is
...
\begin{center}
\caption{{\it%\small
			Different energies as a function of $\omega$ for a $2$ particle quantum dot with the optimal variational parameters. All energies are calculated using $10^7$ samples.
			The energy are plotted for 12 values of $\Delta t$ between $0.01$ and $0.1$.
			...
			Bottom plot: The Hartree-Fock energies as a ratio of the vmc energies $\expec{E_L}$ for each $\omega$. 
		}}
		\label{figblc}
%\vspace{-.8cm}
