...
An other class of sampling algorithms is based on Langevin dynamics, which is describing the dynamical properties of brownian particles following the Fokker-Planck equation


 suggestion density on the form
\begin{align}
	g(\ts r_{n+1}|\ts r_n)=D\frac{\nabla P(\ts r_n)}{P(\ts r_n)} + \mathcal N(\ts r_{n+1}-\ts r_{n}).	
\end{align}
Here $D$ is a constant 
The transitionkernel is the sum of a drift term proportional to the negative gradient of the , and a normal distributed ... . 
(intuitively smart) Inserted into the M.H. eq.'s ...

.. can be derived in many ways. 1) looking at the system as a diffusional system obeying langevin dynamics (ref). 2) Deriving the qm propagator ... (why is this sensible.)
Not eqact: time ordered product, error propto delta t. problems with to small delta t as well.

\cite{scemama_2006}

 .., where the random walkers ... the Langevin equation for 
...
$N_A,N_B$ start/stop collecting mean value of gradient. At $n=N_B$, $k,l$ is calculated.
At $N_C$ the system should be close to the minima, and we start collecting data $(\alpha,\beta)$.
$v_{min}$ is the lower limit of the variational parameters $\alpha,\beta$. $d_{max}$ is the upper limit of the 
length of the components of the gradient. $k,l$ are dependent on $D,E,F$. $E$ is the (negative) exponent limited by eq. (\ref{eq:sga2}).
I have used $m=30$, $N_A=50$, $N_B=120$, $N_C=200$, $v_{min}=0.1$, $d_{max}=0.1$, $D=300$, $E=0.8$, $F=20$ in all calculations. 
}
\label{minalg}
\end{algorithm}
