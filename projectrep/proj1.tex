...
dynamical equations in time $t\to t_n= \Delta t n$, and an time step error arises. The time step error is quadratic in $\Delta t$ 
and leads to an imperfect sampling of $P$.
The Metropolis-Hastings algorithm corrects this error (sampling part) perfectly for any time step $\Delta t>0$.
...
convergence of the system even though the acceptance rate increases. (Since the random walkers in $\mathcal D$ will be very slow). In practice one 
has to experiment to find a $\Delta$ that gives a small time step error and a rapid convergence. 

Time step error resulting from nodal surfaces..
