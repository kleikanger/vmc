...
\end{align}
When $\gamma_n$ gets smaller the parameters $(\alpha_n,\beta_n)$ will oscillate around the minima with a decreasing amplitude. 

The SGA is based on the Robbins-Monroe theorem which states that under certain mathematical conditions the series (\ref{eq:sga1})
...
Choosing the right $\gamma_n$ is vital for the rate of convergence of the algorithm.
...
I have used a simple algorithm with a $\gamma_n$ on the form
\begin{align}
	\gamma_n=(n_0 + k_n * n)^{-0.8},
\end{align}
where $k_n$ and $n_0$ are parameters that is calculated as a function of the average length of the gradient during an initial thermalization phase.
...
is assumed that whenever the gradient changes its sign, the system is close to it's minima. After the thermalization phase, each of the counters are set to $0$

I have found that the following procedure works well:
\begin{itemize}
\item The maximum step length is set to $0.01$. This stabilizes the simulation during the initial phase when $\gamma_n$ is small and the step length might be large.
	After the thermalization phase the average step length becomes much smaller than $0.01$.
\item $n_0$ is initially chosen to be $0$, and $k_n$ to be $1$.
\item $n$ is updated every time the gradient changes its sign.
\item The $30$ first cycles is thermalization cycles. During the next $40$ cycles the average of the absolute value of the gradient is collected. 
After the $70$ first iterations, $n_0$ is chosen so that the average step length multiplied by $n_0$ is equal to $1/400$. $k_n$ is chosen to be $1/40$ of $n_0$. 
$n$ is reset to $0$.
\item Averages are collected after the system is thermalized and $n_0$ and $k_n$ is calculated.
\end{itemize}
The approximate value of the gradient $\left({\partial_\alpha},{\partial_\beta}\right)\expec{E_L}$ is obtained during a short MC run, with $m=\mathcal O (1000)$.
(This is close to correlation length found during the blocking analysis). I get bad results if $m$ is chosen to be smaller than this. 

The algorithm proved efficient when applied on the full shell quantum dots, generally converging to results with an error close to $(10^{-3})$ in less than $10^6$
total MC cycles. The minimas are very good. To distinguish the energy between minimas that is as close as $10^{-3}$ in $(\alpha,\beta)$, more than $\mathcal O 10^8$ 
cycles are needed. ...
...
Harju et. al. has implemented an SGA algorithm in \cite{harju_2005}. Here the number of cycles $m$ is set to $\mathcal O(10^1)-\mathcal O(10^2)$. My algorithm gives bad results for 
$m$ that is smaller than $\mathcal O(10^3)$. On main difference between his implementation and mine is that he uses 10 (?) uncorrelated walkers or more, while I use only one. 
The high correlation between the samples will lead to bad results if the number of samples is much smaller than the correlation length. Therefore I think that it would be better to 
use a larger number of walkers.
...
\vspace{-.8cm}
\caption{{\it\small 
	Minimization of a $12$ particle quantum dot. $N_C$ is the total number of MC-samples. 
	The red line shows the $\alpha$ and $\beta$ values at $N_C$, while the black line shows the accumulated mean.
	The mean are quickly converging to the optimal results  $\beta=0.877$ and $\alpha=0.657$, only using a total of total of $6\times10^5$ samples. }}
\end{figure}
...
The gradient of the expectationvalue of the energy can be written \cite{lin_2000}
\begin{align}
